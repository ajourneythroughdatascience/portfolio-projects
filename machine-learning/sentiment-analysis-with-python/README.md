...
# Errors
- `ERROR: "source.txt" DOES NOT EXIST`: A `source.txt` file must exist inside the specified `input` directory. This file will contain the URLs required to download data sets. If data sets already exists, use the `Read Mode` instead.
- `ERROR: "COL" DOES NOT EXIST`: The specified column does not exist in the dataset.
