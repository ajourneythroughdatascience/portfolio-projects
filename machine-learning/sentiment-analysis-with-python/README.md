...
Sentiment Analysis with Python implements 4 different pretrained NLP models for Sentiment Analysis. It includes a GUI for selecting which model to run, the possibility to perform analyses on existing `.tsv` & `.tsv.gz` files of choice, the possibility to define a list of URLs to download the target files and use them as input for analyses, and more. 
